...
def _parse_optional_int(value, field_label):
    """Parse optional integer fields from JSON payloads."""
    if value is None:
        return None
    if isinstance(value, str):
        value = value.strip()
...
def get_user(user_id: int):
    ...
    try:
        ...
        user_df = user_df.where(user_df.notna(), None)
...
def create_user():
    ...
    try:
        ...
        try:
            smtp_port = _parse_optional_int(data.get("smtp_port"), "SMTPポート")
        except ValueError as exc:
            return jsonify({"success": False, "error": str(exc)}), 400

        smtp_host = _normalize_optional_text(data.get("smtp_host"))
        smtp_user = _normalize_optional_text(data.get("smtp_user"))
        smtp_password = _normalize_optional_text(data.get("smtp_password"))
...
def update_user(user_id: int):
    ...
    try:
        ...
        if "smtp_host" in data:
            ...
            params["smtp_host"] = _normalize_optional_text(data.get("smtp_host"))

        if "smtp_port" in data:
            try:
                params["smtp_port"] = _parse_optional_int(data.get("smtp_port"), "SMTPポート")
            except ValueError as exc:
                return jsonify({"success": False, "error": str(exc)}), 400
        ...
        if "smtp_user" in data:
            ...
            params["smtp_user"] = _normalize_optional_text(data.get("smtp_user"))

        if "smtp_password" in data:
            update_fields.append("smtp_password = :smtp_password")
            params["smtp_password"] = _normalize_optional_text(data.get("smtp_password"))
