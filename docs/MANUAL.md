...
1. [初回セットアップ（CA証明書のインストール）](#初回セットアップca証明書のインストール)
2. [システム概要](#システム概要)
3. [ログイン](#ログイン)
4. [在庫一覧](#在庫一覧)
5. [入出庫管理](#入出庫管理)
...
## 初回セットアップ（CA証明書のインストール）

本システムはHTTPS（暗号化通信）で動作しています。**初めてアクセスする端末には、CA証明書のインストールが必要**です。インストールしないとブラウザがセキュリティ警告を表示します。

### CA証明書のダウンロード

以下のURLからCA証明書ファイル（`syomohin-ca.crt`）をダウンロードしてください：

```
https://daisoind.sharepoint.com/:u:/s/msteams_24d065/IQA7BHaCVHEnQ486anVNG5xQAb1caLUJHxle1ZdbRnXTb7Q?e=ntrYkk
```

### Windowsへのインストール手順

1. ダウンロードした `syomohin-ca.crt` をダブルクリック
2. 「証明書のインストール」ボタンをクリック
3. 「ローカルコンピューター」を選択 → 「次へ」
4. 「証明書をすべて次のストアに配置する」を選択 → 「参照」
5. **「信頼されたルート証明機関」** を選択 → 「OK」
6. 「次へ」→「完了」
7. セキュリティの警告が出たら「はい」→ インストール完了
8. **ブラウザを完全に閉じて再起動**

インストール後、`https://10.0.1.232:8504` にアクセスすると警告なしで表示されます。
...
- **アクセスURL**: `https://10.0.1.232:8504`
- **通信**: HTTPS（暗号化）※初回はCA証明書のインストールが必要（[手順はこちら](#初回セットアップca証明書のインストール)）
...
### Q0: ブラウザに「この接続は安全ではありません」と表示される
**A:** CA証明書がインストールされていません。[「初回セットアップ」](#初回セットアップca証明書のインストール)の手順でインストールしてください。
...
**マニュアル作成日**: 2025-11-15
**最終更新日**: 2026-05-14（HTTPS対応・CA証明書インストール手順追加）
**作成者**: システム管理者
**バージョン**: 2.0
